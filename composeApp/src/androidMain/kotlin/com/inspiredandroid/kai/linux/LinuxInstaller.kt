...
class LinuxInstaller(private val paths: LinuxPaths) {
    ...
    private val downloader = RootfsDownloader(httpClient)
     ...
     */
    suspend fun install(distro: LinuxDistro, onStep: (InstallStep) -> Unit): InstallMarker = packageLock.withLock {
        ...
        // Clear any stale package manager lock files left by interrupted operations
        listOf(
            "var/lib/dpkg/lock",
            "var/lib/dpkg/lock-frontend",
            "var/lib/apt/lists/lock",
            "var/cache/apt/archives/lock",
        ).forEach { File(paths.rootfsDir, it).delete() }
