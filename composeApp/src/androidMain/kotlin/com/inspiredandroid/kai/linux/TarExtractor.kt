...
object TarExtractor {
    ...
    private fun extractTar(inputStream: InputStream, targetDir: File) {
        ...
        while (true) {
            ...
            val rawName = readTarString(headerBuffer, TAR_NAME_OFFSET, 100)
            if (rawName.isEmpty()) break

            val prefix = readTarString(headerBuffer, TAR_PREFIX_OFFSET, 155)
            val headerFullName = if (prefix.isNotEmpty()) "$prefix/$rawName" else rawName
            ...
            val rawLinkName = readTarString(headerBuffer, TAR_LINK_OFFSET, 100)

            // Handle GNU Tar / PAX long name headers
            if (type == 'L') {
            ...
            if (type == 'K') {
                ...
                readFully(inputStream, longLinkBytes)
            ...
            val fullName = nextLongName ?: headerFullName
            val linkName = nextLongLink ?: rawLinkName
            nextLongName = null
            nextLongLink = null
            ...
            // Non-file entries (pax records, unknown flags) still carry a body.
