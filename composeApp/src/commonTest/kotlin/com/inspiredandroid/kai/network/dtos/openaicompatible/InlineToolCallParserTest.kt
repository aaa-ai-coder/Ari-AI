...
class InlineToolCallParserTest {
    ...
    @Test
    fun `parses json flavor with string arguments field without double escaping`() {
        val raw = """before<tool_call>{"name":"execute_shell_command","arguments":"{\"command\":\"ls -la\",\"timeout\":10}"}</tool_call>after"""
        ...
        val call = extracted.calls[0]
        assertEquals("execute_shell_command", call.name)
        assertEquals("{\"command\":\"ls -la\",\"timeout\":10}", call.arguments)
        assertEquals("beforeafter", extracted.cleanedText)
    }
