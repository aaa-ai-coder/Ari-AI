...
class BuildTerminalTextTest {
    ...
    fun textMatchesPerCellFlattening() {
        ...
        assertEquals(flattenPerCell(snap), buildTerminalText(snap, TerminalTheme.CLASSIC_DARK).text)
    }
    ...
    fun collapsesUniformRowsIntoOneSpanEach() {
        ...
        val spans = buildTerminalText(snap, TerminalTheme.CLASSIC_DARK).spanStyles.size
    ...
    fun cursorCellGetsItsOwnInvertedSpan() {
        ...
        val snap = screen.snapshot()
        val theme = TerminalTheme.CLASSIC_DARK

        val cursor = buildTerminalText(snap, theme).spanStyles
            .single { it.start == snap.cursorCol && it.end == snap.cursorCol + 1 }
        assertEquals(theme.background, cursor.item.color)
        assertEquals(theme.foreground, cursor.item.background)
    }
    ...
    fun colorChangeSplitsTheRun() {
        ...
        val snap = screen.snapshot()
        val theme = TerminalTheme.CLASSIC_DARK
        val spans = buildTerminalText(snap, theme).spanStyles

        val red = spans.single { it.start == 0 }
        assertEquals(theme.palette[1], red.item.color)
        assertEquals(2, red.end)

        val green = spans.single { it.start == 2 }
        assertEquals(theme.palette[2], green.item.color)
