...
internal class VtParser(private val screen: TerminalScreen) {
        ...
        while (i < text.length) {
            ...
            when (state) {
                ...
                State.Csi -> when {
                    ...
                    ch.code in 0x20..0x3F || ch == ';' || ch == ':' || ch in '0'..'9' || ch == '?' || ch == '>' || ch == '=' || ch == ' ' || ch == '!' || ch == '"' || ch == '$' || ch == '\'' -> {
                        csiParams.append(ch)
                    }
                    else -> {
                        // malformed — abort
    ...
    private fun dispatchCsi(final: Char, raw: String) {
        ...
        when (final) {
            ...
            'S' -> screen.scrollUp(params.default(1))
            'T' -> screen.scrollDown(params.default(1))
            'r' -> {
                if (params.size >= 2 && params[0] > 0 && params[1] > 0) {
                    screen.setScrollRegion(params[0] - 1, params[1] - 1)
                } else {
    ...
    private fun parseParams(raw: String): List<Int> {
        ...
        // Support both semicolons and colons (for TrueColor 38:2::r:g:b format)
