...
object AptPackageManager : PackageManagerSpec {
    ...
    override fun installCommand(name: String): String {
        val quoted = name.split("\\s+".toRegex()).filter { it.isNotBlank() }.joinToString(" ") { shellQuote(it) }
        return "apt-get install -y --no-install-recommends $quoted"
    }

    override fun removeCommand(name: String): String {
        val quoted = name.split("\\s+".toRegex()).filter { it.isNotBlank() }.joinToString(" ") { shellQuote(it) }
        return "apt-get remove -y $quoted"
    }
