...
private val DOCKERFILE_KEYWORDS = setOf(
    "FROM", "RUN", "CMD", "LABEL", "EXPOSE", "ENV", "ADD", "COPY", "ENTRYPOINT",
    "VOLUME", "USER", "WORKDIR", "ARG", "ONBUILD", "STOPSIGNAL", "HEALTHCHECK", "SHELL",
    "from", "run", "cmd", "label", "expose", "env", "add", "copy", "entrypoint",
    "volume", "user", "workdir", "arg", "onbuild", "stopsignal", "healthcheck", "shell", "AS", "as",
)

private val YAML_KEYWORDS = setOf(
...
private val KEYWORD_REGEXES: Map<String, Regex> = buildMap {
    ...
    put("h", c)
    put("hpp", c)
    val docker = buildKeywordRegex(DOCKERFILE_KEYWORDS)
    put("dockerfile", docker)
    put("docker", docker)
    val yaml = buildKeywordRegex(YAML_KEYWORDS)
    put("yaml", yaml)
    put("yml", yaml)
    put("toml", yaml)
    put("json", yaml)
...
internal fun highlightCode(code: String, language: String?, colors: HighlightColors): AnnotatedString {
    ...
    val commentRegex = when (lang) {
        "python", "py", "bash", "sh", "shell", "zsh", "yaml", "yml", "dockerfile", "docker", "toml" -> HASH_COMMENT_REGEX
        "xml", "html" -> null
